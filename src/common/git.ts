...
export class Gitter {
  ...
  public async cloneOrPull(
    ...
    "updated" | "cloned" | "skipped" | { type: "error"; message: string }
  > {
    return this.exists(repo)
      ? this.pull(repo, defaultBranch, silent)
      : this.clone(repo, silent, shallow);
  }
  ...
  private async pull(
    ...
    if (this.type === "cache") {
      ...
      try {
        const currentBranch = await repoClient.revparse([
          "--abbrev-ref",
          "HEAD",
        ]);
    ...
    if (!silent) {
      log(
        `${repo}, exists, pulled OK (${Math.round(performance.now() - t1)}ms)`,
      );
    }
  ...
  public async clone(
    ...
    if (!silent) {
      log(
        `Cloned ${repo}${shallow ? " (shallow)" : ""} OK (${Math.round(performance.now() - t1)}ms))`,
      );
    }
