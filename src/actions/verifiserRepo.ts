...
export async function verifiserRepo(r: RepoConfig) {
  ...
  function verifiser(key: string, forventet: any) {
    if ((repo.data as any)[key] !== forventet) {
      ok = false;
      log(
        `${repo.data.full_name} ${key} har ikke forventet verdi: ${forventet}`,
      );
    }
  }
...
async function verifiserTopic(r: RepoConfig, repo: OctokitResponse<any>) {
  ...
  if ((repo.data.topics as string[]).includes("team-esyfo")) {
    ...
    log(
      chalk.red(
        `${repo.data.full_name} mangler team-esyfo topic. ${foreslaaPatch}`,
      ),
    );
    if (r.patch) {
...
async function verifiserAdminTeams(r: RepoConfig) {
  ...
  const repoTeams = await getOctokitClient().request(
  ...
  for (const team of adminTeams) {
    if (!aksepterteTeams.includes(team)) {
      log(
        chalk.red(
          `Team ${team} har admin tilgang til ${r.name}. Du må fjerne dem i GitHub manuelt`,
        ),
      );
      process.exit(1);
    }
  ...
  for (const team of aksepterteTeams) {
    if (!adminTeams.includes(team)) {
      ...
      if (r.patch) {
        log(
          chalk.cyan(
            "Gir admin tilgang til team: " + team + " for repo: " + r.name + "",
          ),
        );
        await getOctokitClient().request(
...
async function verifiserDefaultBranchProtection(
  ...
  try {
    ...
    verifiser(
      "enforce_admins",
      "enabled",
      !skipEnforceAdmin.includes(repo.name),
    );
    ...
    if (ok) {
      ...
      const foreslaaPatch = !repo.patch
        ? ` Kjør med -p flagg for å fikse.`
        : ``;
      log(
        chalk.red(
          `Repo ${repo.name} har feil branch protection. ${foreslaaPatch}`,
        ),
      );
    }
  } catch (e) {
    ...
    log(
      chalk.red(
        `Repo har ikke satt opp branch protection. ${foreslaaPatch}`,
        e,
      ),
    );
  }
...
async function verifiserLabels(repo: RepoConfig) {
  const labels = await getOctokitClient().request(
  ...
  if (!labels.data.map((l) => l.name).includes("automerge")) {
    ...
    log(
      chalk.red(
        `Mangler automerge label i repoet ${repo.name}. ${foreslaaPatch}`,
      ),
    );
    if (repo.patch) {
