...
export async function copilotSetup(): Promise<void> {
    ...
    log(chalk.yellow('  Agenter distribueres nå automatisk til hvert repo via copilot sync,'))
    log(chalk.yellow('  og trenger ikke lenger installeres lokalt som plugin.\n'))
    log('  Kjør i stedet:')
...
async function cleanupOldPluginFiles(): Promise<void> {
    ...
    const pluginDir = path.join(home, '.copilot', 'installed-plugins', '_direct', 'esyfo-agents')
