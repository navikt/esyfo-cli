...
import type { RepoProfile } from "./sync-config.ts";
...
async function detectKotlinStack(
  repoPath: string,
  stack: RepoStackInfo,
): Promise<void> {
  ...
  stack.hasDatabase =
    ...
    content.includes("flyway") ||
    content.includes("hikari");
  if (
...
async function detectTypeScriptStack(
  repoPath: string,
  stack: RepoStackInfo,
): Promise<void> {
...
async function detectMonorepoProfiles(
  repoPath: string,
): Promise<RepoProfile[]> {
  const pkgFile = Bun.file(path.join(repoPath, "package.json"));
  if (!(await pkgFile.exists())) return [];
...
async function enrichMonorepoStack(
  repoPath: string,
  stack: RepoStackInfo,
): Promise<void> {
  const appDirs = resolveMonorepoAppDirs(repoPath);
  ...
  for (const dir of appDirs) {
    if (await fileExists(path.join(dir, "build.gradle.kts"))) {
      ...
      if (sub.testingLib && !stack.testingLib)
        stack.testingLib = sub.testingLib;
...
async function directoryHasNaisConfig(repoPath: string): Promise<boolean> {
  ...
  const [hasYaml, hasYml] = await Promise.all([
    fileExists(naisYaml),
    fileExists(naisYml),
  ]);
...
export function logStackInfo(repoName: string, stack: RepoStackInfo): void {
  ...
  if (stack.hasKafka)
    parts.push(stack.kafkaLib ? `Kafka (${stack.kafkaLib})` : "Kafka");
  log(`  ${parts.join(" · ")}`);
}
