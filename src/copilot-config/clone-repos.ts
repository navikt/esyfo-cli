...
export async function cloneRepos<TRepo extends CloneRepoInput>(repos: TRepo[]): Promise<CloneReposResult<TRepo>> {
    log(chalk.green('Cloning/pulling repositories...'))

    const gitter = new Gitter('cache')
    const cloneResults = await Promise.allSettled(
        repos.map((repo) => gitter.cloneOrPull(repo.name, repo.defaultBranch, true)),
    )
